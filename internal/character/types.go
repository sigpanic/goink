...
type CharacterRelation struct {
	...
	ChapterNumber     int       `gorm:"column:chapter_number"                      json:"chapter_number"`       // 此关系在哪个章节确立/变更，可用于拉取章节细节作为 LLM 上下文；v1.6.0 迁移期保留，commit 1.7 删除
	ChapterID         *int64    `gorm:"column:chapter_id;index"                     json:"chapter_id"`           // v1.6.0 新增：chapters.id 外键（nullable）；commit 1.5 由 ChapterNumber 反查填充
