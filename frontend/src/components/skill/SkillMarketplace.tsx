...
import { ArrowLeft, Loader2, RefreshCw, Store, X, AlertTriangle, ExternalLink } from 'lucide-react'
...
function classifyError(code: string, msg: string, t: TFunction): { code: string; message: string } {
  // 后端 err_code 是带模块前缀的全码（如 "githubapi.rate_limited"、"llm.not_found"），
  // 用 endsWith 匹配短码，保持 canRetry 等下游逻辑用短码判断。
  if (code.endsWith('network')) return { code: 'network', message: t('skill.marketplace.errorNetwork') }
  if (code.endsWith('rate_limited')) return { code: 'rate_limited', message: t('skill.marketplace.errorRateLimited') }
  if (code.endsWith('not_found')) return { code: 'not_found', message: t('skill.marketplace.errorNotFound') }
  if (code.endsWith('forbidden')) return { code: 'forbidden', message: t('skill.marketplace.errorForbidden') }
  return { code: 'other', message: t('skill.marketplace.errorOther', { message: msg || code }) }
}

export default function SkillMarketplace({ open, onOpenChange, novelId, onInstalled }: Props) {
  const app = useApp()
  ...
  const [selectedSkill, setSelectedSkill] = useState<remote.RemoteSkillMeta | null>(null)
  ...
  const loadRemote = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await app.ListRemoteSkills({ page, size: pageSize, query: debouncedQuery })
      ...
      setError({ code: 'other', message: t('skill.marketplace.errorOther', { message: msg }) })
      setItems([])
      setTotal(0)
      setTotalPages(0)
    } finally {
  ...
  const loadRemoteContent = useCallback(async (name: string) => {
  ...
  const probeLocal = useCallback(async (target: InstallTarget, name: string): Promise<string> => {
  ...
  // Install skill
  const doInstall = useCallback(async (target: InstallTarget, name: string) => {
    setInstalling(true)
    try {
      const res = await app.InstallRemoteSkill({ name, target, novel_id: novelId })
      const code = res?.err_code ?? ''
      if (code && code !== 'ok') {
        const cls = classifyError(code, res?.err_msg ?? '', t)
  ...
  const handleInstall = useCallback(async (target: InstallTarget) => {
  ...
  }, [selectedSkill, novelId, t, probeLocal, remoteContent, loadRemoteContent, doInstall])
  ...
  useEffect(() => {
    if (phase === 'confirm_overwrite' && remoteContent && !remoteContentForConfirm) {
      setRemoteContentForConfirm(remoteContent)
    }
  ...
  const remoteSplit = useMemo(() => splitFrontmatter(remoteContent), [remoteContent])
  const remoteForConfirmSplit = useMemo(() => splitFrontmatter(remoteContentForConfirm), [remoteContentForConfirm])
  const localSplit = useMemo(() => splitFrontmatter(localContent), [localContent])

  if (!open) return null

  const canRetry = error && (error.code === 'network' || error.code === 'rate_limited' || error.code === 'other')
        ...
        {phase === 'browse' && (
          <div className="flex items-center justify-between px-6 py-4 border-b shrink-0 gap-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-semibold">{t('skill.marketplace.title')}</h2>
            </div>
            <div className="flex items-center gap-2">
        ...
        {phase === 'detail' && selectedSkill && (
          <div className="flex items-center justify-between px-6 py-4 border-b shrink-0 gap-3">
            <div className="flex items-center gap-2 min-w-0">
              <button
                onClick={() => { setPhase('browse'); setSelectedSkill(null); setRemoteContent(''); setLocalContent(''); setContentError('') }}
              ...
              <h2 className="text-base font-semibold truncate">
                {selectedSkill.name}
                <span className="ml-2 text-xs text-muted-foreground font-normal">v{selectedSkill.version}</span>
              </h2>
            </div>
        ...
        {phase === 'confirm_overwrite' && selectedSkill && (
          <div className="flex items-center justify-between px-6 py-4 border-b shrink-0 gap-3">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <h2 className="text-base font-semibold truncate">{t('skill.marketplace.overwriteTitle')}</h2>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={() => setPhase('detail')}
        ...
        {phase === 'browse' && (
          <div className="flex-1 min-h-0 flex flex-col">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2 gap-2">
                          <h3 className="text-base font-semibold text-foreground truncate flex-1">{sk.name}</h3>
                          <span className="text-xs text-muted-foreground shrink-0">v{sk.version}</span>
                        </div>
                        ...
                        <div className="flex items-center flex-wrap gap-1.5 text-[11px] text-muted-foreground/80 mt-auto">
                          ...
                          {sk.mode && (
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md font-medium ${modeClass(sk.mode)}`}>
                              {modeLabel(t, sk.mode)}
                            </span>
                          )}
                          {sk.author && (
                            <span className="inline-block px-1 py-0.5 truncate">· {sk.author}</span>
                          )}
                        </div>
            ...
            <div className="flex items-center justify-between px-6 py-3 border-t shrink-0 text-xs text-muted-foreground">
              <div>
                {total > 0
                  ? t('skill.marketplace.pagination', { total, page, totalPages, count: total })
                  : ''}
              </div>
              <div className="flex items-center gap-2">
                <select
                    ...
                    <tbody>
                      {remoteSplit.meta.name && (
                        <tr className="border-b">
                          <td className="px-4 py-2 text-muted-foreground whitespace-nowrap w-20">{t('skill.marketplace.fieldName')}</td>
                          <td className="px-4 py-2 text-foreground font-semibold">{remoteSplit.meta.name}</td>
                        </tr>
                      )}
                      {remoteSplit.meta.description && (
                        <tr className="border-b">
                          <td className="px-4 py-2 text-muted-foreground whitespace-nowrap w-20">{t('skill.marketplace.fieldDescription')}</td>
                          <td className="px-4 py-2 text-foreground">{remoteSplit.meta.description}</td>
                        </tr>
                      )}
                      {remoteSplit.meta.category && (
                        <tr className="border-b">
                          <td className="px-4 py-2 text-muted-foreground whitespace-nowrap w-20">{t('skill.marketplace.fieldCategory')}</td>
                          <td className="px-4 py-2 text-foreground">{remoteSplit.meta.category}</td>
                        </tr>
                      )}
                      {remoteSplit.meta.mode && (
                        <tr className="border-b">
                          <td className="px-4 py-2 text-muted-foreground whitespace-nowrap w-20">{t('skill.marketplace.fieldMode')}</td>
                          <td className="px-4 py-2">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium ${modeClass(remoteSplit.meta.mode)}`}>
                      ...
                      {remoteSplit.meta.author && (
                        <tr className="border-b">
                          <td className="px-4 py-2 text-muted-foreground whitespace-nowrap w-20">{t('skill.marketplace.fieldAuthor')}</td>
                          <td className="px-4 py-2 text-foreground">{remoteSplit.meta.author}</td>
                        </tr>
                      )}
                      {remoteSplit.meta.version && (
                        <tr>
                          <td className="px-4 py-2 text-muted-foreground whitespace-nowrap w-20">{t('skill.marketplace.fieldVersion')}</td>
                          <td className="px-4 py-2 text-foreground">v{remoteSplit.meta.version}</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
              ...
              <div className="border rounded-lg overflow-hidden flex flex-col min-h-0">
                ...
                <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-2">
                  {localContent ? (
                    ...
                    <div className="text-xs text-muted-foreground">{t('skill.marketplace.noLocalContent')}</div>
                  )}
                </div>
              </div>
              <div className="border rounded-lg overflow-hidden flex flex-col min-h-0">
...
function CompareView({ split, t }: {
  ...
  if (meta.description) rows.push([t('skill.marketplace.fieldDescription'), meta.description])
  if (meta.category) rows.push([t('skill.marketplace.fieldCategory'), meta.category])
  ...
  if (meta.version) rows.push([t('skill.marketplace.fieldVersion'), 'v' + meta.version])
  return (
    <>
      {rows.length > 0 && (
          ...
          <tbody>
            {rows.map(([k, v], i) => (
              <tr key={k} className={i < rows.length - 1 ? 'border-b' : ''}>
                <td className="px-2 py-1 text-muted-foreground whitespace-nowrap w-16 align-top">{k}</td>
