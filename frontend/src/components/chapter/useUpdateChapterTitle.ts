...
// mutationFn 直接 import wailsjs UpdateChapterTitle（不用 useApp），3 参 (novelId, chapterID, title)。
// §6 全量回传：入参 {chapterID, title} 完整传入（UpdateChapterTitle 入参就是这两个，无 patch 语义）。
